...
| 9 | Cache y CDN | 5 | parcial | `vercel.json` cachea agresivamente (`immutable`, 1 año) los bundles con hash de contenido bajo `/assets/*` y `/restaurantes/assets/*`; el HTML de entrada sigue `no-cache` a propósito. Falta invalidación de imágenes/media no versionadas y una capa de caché de aplicación (no solo hosting). |
...
| QUAL-01 | medio | corregido local (parcial) | TypeScript sigue permisivo y lint conserva 34 warnings. | 8-sep: lint bajó de 36 (recuento actualizado) a 10 warnings reales — 9 `any` reemplazados por tipos reales derivados de Supabase, 3 dependencias de hooks corregidas sin introducir bucles, 8 warnings `react-refresh/only-export-components` resueltos separando helpers no-componente de shadcn/ui a archivos propios. Verificado con `tsc -p tsconfig.app.json --noEmit` (el `tsc --noEmit` de la raíz es un no-op: `tsconfig.json` tiene `files: []`), `npm run lint` y `npm run build` reales, y push desplegado en verde. Quedan 10 warnings preexistentes fuera de este alcance (`any` en 4 componentes de admin, 2 dependencias de hooks, 1 eslint-disable sin uso) y el modo `strict` de TypeScript sigue sin activarse por etapas. |
