alter table public.orders add column if not exists dedupe_fingerprint text;
alter table public.orders add column if not exists idempotency_key text;

alter table public.orders drop constraint if exists orders_dedupe_fingerprint_format;
alter table public.orders add constraint orders_dedupe_fingerprint_format
...
create unique index if not exists orders_tenant_idempotency_key_uidx
  on public.orders(restaurant_id, idempotency_key)
  where idempotency_key is not null;
create index if not exists orders_tenant_fingerprint_created_idx
  on public.orders(restaurant_id, dedupe_fingerprint, created_at desc)
