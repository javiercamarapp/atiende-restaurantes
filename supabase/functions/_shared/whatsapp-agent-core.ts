...
export const TONE_INSTRUCTIONS: Record<string, string> = {
  calido_cercano:
    "Cálido y cercano: como el encargado de confianza de la sucursal que ya conoce al cliente — cercano mexicano, sin ser cursi.",
  formal_directo:
    "Formal y directo: cortés y profesional, sin diminutivos ni emojis, va al grano en cada mensaje.",
  profesional_neutro:
...
export const TOOLS = [
  {
    ...
    function: {
      ...
      description:
        "Dado el nombre de una colonia/zona/referencia que dio el cliente, devuelve la sucursal real MÁS CERCANA calculada por distancia real (no adivines tú cuál está más cerca). Llámala en cuanto tengas la colonia o una referencia clara, antes de decirle al cliente de qué sucursal va a salir su pedido.",
      parameters: {
        type: "object",
        properties: {
          colonia: {
            type: "string",
            description:
              "La colonia, zona o referencia que dio el cliente, tal cual (ej. 'Cholul', 'cerca de Plaza Las Américas'). Puede ser una frase, no hace falta que sea solo el nombre exacto.",
          },
        },
        required: ["colonia"],
      },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          query: {
            type: "string",
            description: "texto a buscar, ej. 'pastor' o 'kilo arrachera'",
          },
          branch_slug: {
            type: "string",
            description:
              "El branch_slug de la sucursal ya confirmada en el paso 3. Si todavía no se confirma la sucursal, no llames esta herramienta.",
          },
        },
  ...
  },
  {
    ...
    function: {
      name: "crear_pedido",
      description:
        "Registra el pedido final en el sistema. Solo llamar cuando el cliente ya confirmó todo, incluyendo la sucursal.",
      parameters: {
        type: "object",
        properties: {
          branch_slug: {
            type: "string",
            description:
              "El branch_slug real de la sucursal más cercana, de la lista de sucursales del prompt.",
          },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Registra nombre/motivo de un mensaje que NO es para hacer un pedido, para que alguien del restaurante le regrese la llamada. Nunca usar para pedidos normales.",
      parameters: {
        type: "object",
        properties: {
          customer_name: { type: "string" },
          reason: {
            type: "string",
            description: "Motivo breve, ej. 'queja', 'facturación', 'empleo'.",
          },
          message: {
            type: "string",
            description: "Resumen breve de lo que dijo el cliente.",
          },
        },
...
export async function runAgentTurn(
  ...
  for (let turn = 0; turn < 4; turn++) {
    ...
    for (const call of toolCalls) {
      ...
      if (result === undefined) {
        try {
          if (call.function.name === "buscar_sucursal_cercana") {
            const { data: matches, error: sucursalError } = await supabase.rpc(
      ...
      if (
        call.function.name === "crear_pedido" &&
        result &&
